...
// Helper function to check if employee has missing info
...
const invalidEmployees = (employees) => {
  ...
  for(let employee of employees) {
    if(employeeWithMissingInformation(employee)) {
      invalids.push(employee); 
    }
  }

  return invalids.map(emp => emp.employee_name);
}

// Usage
const names = invalidEmployees(sampleEmployees); 
console.log(names);